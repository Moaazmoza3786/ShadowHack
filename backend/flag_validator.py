...
class FlagValidator:
    ...
    # Legacy flags from study-hub-backend
    LEGACY_FLAGS = {
...
def register_flag_routes(app):
    ...
    def verify_flag_legacy():
        """Legacy compatibility endpoint for study-hub-backend (FastAPI)"""
        data = request.get_json()
        if not data:
        ...
        # Check in legacy flags first
        correct_flag = flag_validator.LEGACY_FLAGS.get(machine_id)
        ...
        # Fallback to standard lab validation if machine_id looks like a lab ID
        try:
            lab_id = int(machine_id)
            is_correct, message, _ = flag_validator.validate_flag(lab_id, submitted_flag, user_id)
            return jsonify({
                "valid": is_correct,
                "message": message,
                "xp": 100 if is_correct else 0
            })
        except (ValueError, TypeError, Exception):
            return jsonify({"valid": False, "message": "Machine ID not found"}), 404
