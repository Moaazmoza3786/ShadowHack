...
@api.route('/codespaces/register', methods=['POST'])
def register_codespace():
    """Register or update a codespace environment"""
    data = request.get_json()
    c_id = data.get('codespace_id')
    user_id = data.get('user_id')
    
    if not c_id:
        return jsonify({"success": False, "message": "codespace_id required"}), 400
        
    ACTIVE_CODESPACES[c_id] = {
        "user_id": user_id,
    ...
    return jsonify({"success": True, "message": "Environment registered"})

@api.route('/codespaces/active', methods=['GET'])
def get_active_codespaces():
    """Admin endpoint to see all live hacking environments"""
    return jsonify({
        "success": True,
