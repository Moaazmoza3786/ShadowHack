...
if not JWT_SECRET:
    raise ValueError(
        "JWT_SECRET environment variable is required! Check your .env file."
    )
JWT_ALGORITHM = "HS256"
...
def register():
    ...
    try:
        ...
        db.session.rollback()
        logger.error("Registration error for user '%s': %s", username, e, exc_info=True)
        return jsonify(
            {
                "success": False,
...
def get_profile():
    ...
    """
    return jsonify(
        {"success": True, "user": request.current_user.to_dict(include_email=True)}
    )
...
def update_profile():
    ...
    try:
        ...
        db.session.rollback()
        logger.error(
            "Profile update error for user id %s: %s", user.id, e, exc_info=True
        )
        return jsonify(
            {
                "success": False,
                "error": "Failed to update profile due to an internal error.",
            }
        ), 500
...
def verify_token():
    ...
    if "error" in payload:
        return jsonify({"valid": False, "error": payload["error"]})

    user = User.query.get(payload.get("user_id"))

    if not user or not user.is_active:
        return jsonify({"valid": False, "error": "User not found or inactive"})
...
def verify_access():
    ...
    Validate the platform access code (invite-only gate).

    The expected code is read from the PLATFORM_ACCESS_CODE environment variable.
    If the variable is not set, access is granted to everyone (open mode).
    ...
    expected_code = os.environ.get("PLATFORM_ACCESS_CODE", "")

    # If no access code is configured, the platform is open — grant access.
    if not expected_code:
        return jsonify({"success": True, "message": "Platform is open access."})
    ...
    # Log failed attempt (IP only — no code value to avoid leaking it in logs)
    logger.warning("Failed platform access attempt from IP: %s", request.remote_addr)
